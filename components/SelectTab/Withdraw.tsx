...
export default function Withdraw({ token0, token1, amm, currentAccount, updateDetails }: Props) {
	...
	const getMaxShare = useCallback(async () => {
		...
		try {
			...
			const shareWithoutPrecision = formatWithoutPrecision(shareWithPrecision, amm.sharePrecision);
	...
	const leftLessThanRightAsBigNumber = (left: string, right: string): boolean => {
	...
	const getEstimate = async (
		...
		try {
			const shareWithPrecision = formatWithPrecision(amountOfShare, amm.sharePrecision);
